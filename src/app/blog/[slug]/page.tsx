    ...
    <div className="mx-auto max-w-5xl px-4 py-12">
            ...
            <div className="p-6">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-bold px-2 py-1 rounded-full"
                  style={{ background: i === 0 ? 'rgba(0,212,255,0.15)' : 'rgba(255,184,0,0.15)', color: i === 0 ? '#00D4FF' : '#FFB800', border: `1px solid ${i === 0 ? 'rgba(0,212,255,0.3)' : 'rgba(255,184,0,0.3)'}` }}>{i === 0 ? 'Nuestra elección' : 'Alternativa'}</span>
              </div>
              ...
              <div className="mt-4 pt-4" style={{ borderTop: '1px solid #1E3A5F' }}><span className="text-sm font-medium flex items-center gap-1" style={{ color: '#00D4FF' }}>Ver oferta <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg></span></div>
            </div>
          </a>
        ))}
      </div>
      ...
      <div className="rounded-2xl p-6 mb-10" style={{ background: '#111827', border: '1px solid #1E3A5F' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-xl p-4" style={{ background: 'rgba(0,212,255,0.05)', border: '1px solid rgba(0,212,255,0.15)' }}>
            <div className="font-bold mb-2" style={{ color: '#00D4FF' }}>Compra {titles[0]} si...</div>
            <ul className="space-y-1 text-sm" style={{ color: '#8BA3C7' }}><li>Pescas más de 30 jornadas al año</li><li>Valoras la suavidad y durabilidad</li><li>Pesca en agua salada</li><li>Presupuesto no es limitación</li></ul>
          </div>
          <div className="rounded-xl p-4" style={{ background: 'rgba(255,184,0,0.05)', border: '1px solid rgba(255,184,0,0.15)' }}>
            <div className="font-bold mb-2" style={{ color: '#FFB800' }}>Compra {titles[1]} si...</div>
            <ul className="space-y-1 text-sm" style={{ color: '#8BA3C7' }}><li>Pescas findes o vacaciones</li><li>Empiezas en el spinning</li><li>Necesitas un segundo carrete</li><li>Buscas la mejor relación calidad-precio</li></ul>
          </div>
        </div>
      </div>
      ...
      <div className="text-base leading-relaxed mb-10" style={{ color: '#8BA3C7' }}>
        {sections.map((block, i) => {
          const t = block.trim()
          if (!t) return null
          if (t.startsWith('## ')) return <div key={i} className="flex items-center gap-3 mt-10 mb-4"><div className="w-1 h-8 rounded-full" style={{ background: 'linear-gradient(180deg, #00D4FF, #FFB800)' }} /><h2 className="text-2xl font-bold" style={{ color: '#E8F0FE' }}>{t.replace('## ', '')}</h2></div>
          if (t.startsWith('### ')) return <h3 key={i} className="text-xl font-bold mt-8 mb-3" style={{ color: '#E8F0FE' }}>{t.replace('### ', '')}</h3>
          if (t.startsWith('- ')) return <div key={i} className="flex items-start gap-3 pl-2 my-1"><span className="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#00D4FF' }} /><span>{t.slice(2)}</span></div>
          if (t.startsWith('|') && t.includes('|')) {
            const lines = t.split('\n').filter(r => r.trim()).filter(r => !r.includes('---'))
            if (lines.length < 2) return <p key={i}>{t}</p>
            const hd = lines[0].split('|').filter(x => x.trim()).map(x => x.trim())
            const rows = lines.slice(1).map(r => r.split('|').filter(x => x.trim()).map(x => x.trim()))
            return <div key={i} className="overflow-hidden rounded-xl my-6" style={{ border: '1px solid #1E3A5F' }}>
              <table className="w-full text-sm">
                <thead><tr style={{ background: '#111827' }}>{hd.map((h, j) => <th key={j} className={`px-4 py-3 text-left font-bold ${j > 0 ? 'text-center' : ''}`} style={{ color: '#E8F0FE', borderBottom: '2px solid #1E3A5F' }}>{h}</th>)}</tr></thead>
                <tbody>{rows.map((row, j) => <tr key={j} style={{ borderBottom: '1px solid #1E3A5F', background: j % 2 === 0 ? 'transparent' : '#11182744' }}>{row.map((cell, k) => {
                  const w = cell.trim().startsWith('✅'); const l = cell.trim().startsWith('❌'); const d = cell.replace('✅','').replace('❌','').trim()
                  return <td key={k} className={`px-4 py-2.5 ${k === 0 ? 'font-medium' : 'text-center'}`} style={{ color: w ? '#26DE81' : l ? '#FF4757' : k === 0 ? '#E8F0FE' : '#8BA3C7' }}>
                    {k > 0 && w ? <span className="inline-flex items-center justify-center w-6 h-6 rounded-full" style={{ background: '#26DE8115' }}><svg className="w-4 h-4" fill="#26DE81" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg></span> : null}
                    {k > 0 && l ? <span className="inline-flex items-center justify-center w-6 h-6 rounded-full" style={{ background: '#FF475715' }}><svg className="w-4 h-4" fill="#FF4757" viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg></span> : null}
                    {k === 0 ? d : (!w && !l) ? d : null}
                  </td>})}</tr>)}</tbody>
              </table>
            </div>
          }
          return <p key={i} className="mb-4 leading-relaxed" style={{ lineHeight: '1.85' }}>{t}</p>
        })}
      </div>
          ...
          {[0, 1].map(i => (
            <a key={i} href={`https://www.amazon.es/dp/${asins[i]}`} target="_blank" rel="nofollow sponsored"
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0"><div className="font-semibold text-sm truncate" style={{ color: '#E8F0FE' }}>{titles[i]}</div><div className="text-xs" style={{ color: '#4A6080' }}>Amazon</div></div>
              </div>
